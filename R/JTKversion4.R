...
jtkdist <- function(timepoints,reps=1,normal=FALSE,alt=FALSE, res = list()) {
  ...
  if(alt) {
    ...
    res$JTK.ALT <- list()				                  ###v3.1 container for alternative distributions

    if(lab %in% names(res$JTK.ALT)) {
      alt.id <- match(lab, names(res$JTK.ALT))
      # return(alt.id)
    }
    nn <- sum(tim)
    M <- (nn^2-sum(tim^2))/2
    res$JTK.ALT[[lab]] <- list()
    res$JTK.ALT[[lab]]$MAX <- M
    if(normal) {
      var <- (nn^2*(2*nn+3) -
                sum(tim^2*(2*tim+3)))/72
      res$JTK.ALT[[lab]]$SDV <- sqrt(var)
      res$JTK.ALT[[lab]]$EXV <- M/2
      # return(length(JTK.ALT))
    }
  } else {
    res$JTK.GRP.SIZE <- tim                          # sizes of each replicate group
    res$JTK.NUM.GRPS <- length(tim)                  # timepoints = number of groups
    res$JTK.NUM.VALS <- nn <- sum(tim)               # number of data values (independent of period and lag)
    res$JTK.MAX <- M <- (nn^2-sum(tim^2))/2          # maximum possible jtk statistic
    res$JTK.GRPS <- rep(1:length(tim), ti=tim)	      ### group labels
    res$JTK.DIMS <- c(nn*(nn-1)/2,1)

    if(normal) {
      res$JTK.VAR <- (nn^2*(2*nn+3) -
                     sum(tim^2*(2*tim+3)))/72              # variance of jtk
      res$JTK.SDV <- sqrt(JTK.VAR)                   # standard deviation of jtk
      res$JTK.EXV <- JTK.MAX/2                       # expected value of jtk
      res$JTK.EXACT <- FALSE
...
jtk.init <- function(periods, interval=1, res) {
  res$JTK.INTERVAL <- interval
  res$JTK.PERIODS <- periods
  res$JTK.PERFACTOR <- rep(1:length(periods),ti=periods)

  tim <- res$JTK.GRP.SIZE
  timepoints <- res$JTK.NUM.GRPS
  timerange <- 1:timepoints-1                     # zero-based time indices
  res$JTK.CGOOSV <- list()
  res$JTK.SIGNCOS <- list()
  ...
  for(i in 1:length(periods)) {
    ...
    cgoosv <- array(cgoos,dim=res$JTK.DIMS)
    res$JTK.CGOOSV[[i]] <- matrix(
      ncol=period,nrow=nrow(cgoosv)
    )
    res$JTK.CGOOSV[[i]][,1] <- cgoosv
    ...
    res$JTK.SIGNCOS[[i]] <- matrix(
      ncol=period,nrow=length(cos.s)
    )
    res$JTK.SIGNCOS[[i]][,1] <- cos.s
    ...
    for(j in 2:period) {                          # one-based half-integer lag index j
      ...
      cgoos <- cgoos[lower.tri(cgoos)]
      cgoosv <- array(cgoos,dim=res$JTK.DIMS)
      res$JTK.CGOOSV[[i]][,j] <- cgoosv

      cos.s <- sign(cos.v)[range]
      cos.s <- rep(cos.s,ti=tim[range])
      res$JTK.SIGNCOS[[i]][,j] <- cos.s
    }
  }
  return(res)
}
...
jtkstat <- function(z, res) {
  alt <- any(is.na(z))				              ### flag for handling missing values
  if(alt) {
    tab <- table(res$JTK.GRPS[is.finite(z)])
  ...
  M <- switch(1+alt,			  	              ### maximum possible S score for this distribution
              res$JTK.MAX,
              res$JTK.ALT[[alt.id]]$MAX)

  foosv <- sign(outer(z,z,"-"))
  foosv <- foosv[lower.tri(foosv)]
  dim(foosv) <- res$JTK.DIMS

  res$JTK.CJTK <- list()
  for(i in 1:length(res$JTK.PERIODS)) {
    res$JTK.CJTK[[i]] <- apply(res$JTK.CGOOSV[[i]],2,
                            function(cgoosv) {
                              ...
                              if(res$JTK.EXACT) {
                                ...
                                p <- switch(1+alt,
                                            2*res$JTK.CP[jtki],
                                            2*res$JTK.ALT[[alt.id]]$CP[jtki])
                              } else {
                                p <- switch(1+alt,
                                            2*pnorm(-(jtk-1/2),
                                                    -res$JTK.EXV,res$JTK.SDV),
                                            2*pnorm(-(jtk-1/2),
                                                    -res$JTK.ALT[[alt.id]]$EXV,
                                                    res$JTK.ALT[[alt.id]]$SDV))
                              }
...
jtkx <- function(z, ampci=FALSE, conf=0.8, res) {      ###v3.1 'ampci=TRUE' for calculating amplitude confidence

  res = jtkstat(z, res = res)                                      # calculate p and S for all (period,phase) combos
  pvals <- lapply(res$JTK.CJTK,function(cjtk) {
    return(cjtk[1,])
  ...
  res$JTK.ADJP <- min(padj)                          # global minimum adjusted p-value

  padj <- split(padj,res$JTK.PERFACTOR)
  minpadj <- sapply(padj,min)                     # minimum adjusted p-value for each period

  peris <- which(res$JTK.ADJP==minpadj)               # indices of all optimal periods
  pers <- res$JTK.PERIODS[peris]                      # all optimal periods

  lagis <- lapply(padj[peris],function(z) {
    which(res$JTK.ADJP==z)
  ...
  for(i in 1:length(pers)) {
    per <- pers[i]
    peri <- peris[i]
    cjtk <- res$JTK.CJTK[[peri]]
    sc <- res$JTK.SIGNCOS[[peri]]
    w <- z[1:nrow(sc)]

    res$JTK.AMPFACTOR <- sqrt(2)                          # 1/median(abs(cosine)) used to calculate amplitudes
    w <- (w-hlm(w))*res$JTK.AMPFACTOR
  ...
  res$JTK.PERIOD <- res$JTK.INTERVAL*bestper        	  # period (hours) with max amp
  res$JTK.LAG <- res$JTK.INTERVAL*bestlag           	  # lag (hours) to peak with max amp
  res$JTK.AMP <- max(0,maxamp)                	      # max amp
  res$JTK.TAU <- besttau                             ### v3.1
  res$JTK.AMP.CI <- maxamp.ci				          ### confidence interval for max amp; 'JTK.AMP.CI' is 'c(0,0)' if 'ampci=FALSE'
  res$JTK.AMP.PVAL <- maxamp.pval			          ### p-value for max amp; 'JTK.AMP.PVAL' is '0' if 'ampci=FALSE'
...
#' @param n number of best fitted plots
#' @param periodHours numeric vector, fit cosine curve (time unit)
#' @return a ggplot of cosine curves with different periods.
#' @import ggplot2
#' @export
#' @examples
#' time = seq(1, 72, by = 3)
